...
export class RegexDetector extends BaseDetector {
  ...
  detect(input: DetectionInput): DetectionMatch[] {
    ...
    const browserCountry = langParts.length > 1 ? langParts[langParts.length - 1].toUpperCase() : undefined;
    const fallbackCountries = ['US', 'GB', 'KR', 'DE', 'FR', 'IN', 'CN', 'BR'];
    const fallbackOnlyCountries = fallbackCountries.filter(c => c !== browserCountry);
    ...
    const isOccupied = (startIndex: number, endIndex: number) => {
      return occupiedRanges.some(
        range => Math.max(startIndex, range.start) < Math.min(endIndex, range.end),
      );
    };
    ...
    for (const pii of allCandidates) {
      ...
      if (pii.entityType === 'phone_number') {
        ...
        const digits = normalizedKey.startsWith('+') ? normalizedKey.substring(1) : normalizedKey;
        ...
        const check = (country?: string): boolean => {
          ...
          try {
            const phoneNumber = parsePhoneNumberFromString(pii.value, country as any, metadata);
            const result = phoneNumber ? phoneNumber.isPossible() && (typeof phoneNumber.isValid === 'function' ? phoneNumber.isValid() : true) : false;
